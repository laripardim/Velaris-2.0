...
function lerCapitulo(livro, capitulo) {
    fetch("http://localhost:7000/capitulo/"+livro+"/"+capitulo)
        .then(resp => {
            return resp.json()
        })
        .then(data => {
            ...
            data.forEach((item, index) => {
               // alert(item.titulo_capitulo + item.capitulo+ item.texto);
                    let titulo = document.querySelector(".texto").querySelectorAll("h1")
                    let txt = document.querySelector(".texto").querySelectorAll("p")
                    titulo[0].innerHTML = item.titulo_capitulo
                    txt[0].innerHTML = item.texto 
...
function buscar(texto) {
  ...
  function capitulo(livro, capitulo) {
    fetch("http://localhost:7000/capitulo/"+livro+"/"+capitulo)
        .then(resp => {
            return resp.json()
        })
        .then(data => {
            ...
            data.forEach((item, index) => {
               // alert(item.titulo_capitulo + item.capitulo+ item.texto);
                    let titulo = document.querySelector(".texto").querySelectorAll("h1")
                    let txt = document.querySelector(".texto").querySelectorAll("p")
                    let capitulo = document.querySelector(".capitulos").querySelectorAll("b")
                    titulo[0].innerHTML = item.titulo_capitulo
                    txt[0].innerHTML = item.texto 

                    fetch("http://localhost:7000/capitulo/"+livro)
                    .then(resp => {
                        return resp.json()
                    })
                    .then(data => {
                        ...
                        let texto = document.querySelectorAll('.texto');
                        data.forEach((item, index) => {
                                capitulo[index].innerHTML = item.capitulo + ' | '
                               capitulo[index].addEventListener("click", () => {
                                   lerCapitulo(livro,item.capitulo)
                                });
                    })
                })
            })
